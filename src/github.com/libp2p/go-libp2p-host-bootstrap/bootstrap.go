...
var DefaultBootstrapConfig = BootstrapConfig{
	...
	BootstrapPeers:    DefaultBootstrapPeerInfos,
}
...
func DefaultBootstrapPeerInfos() []pstore.PeerInfo {
	p, err := DefaultBootstrapPeers()
	if err != nil {
		panic(err)
	}
	return toPeerInfos(p)
}
...
func Bootstrap(ctx context.Context, peerHost p2phost.Host, id peer.ID, cfg BootstrapConfig) error {
	...
	go func() {
		for {
			timeout, _ := context.WithTimeout(ctx, cfg.Period)
			defer log.EventBegin(timeout, "periodicBootstrap", id).Done()

			if err := bootstrapRound(timeout, peerHost, cfg); err != nil {
				log.Event(timeout, "bootstrapError", id, lgbl.Error(err))
...
func toPeerInfos(bpeers []BootstrapPeer) []pstore.PeerInfo {
